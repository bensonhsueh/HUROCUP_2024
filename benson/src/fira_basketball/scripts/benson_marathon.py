...
DEBUG_MODE = True
DEBUG_MODE = False # Show detected image
DEBUG_MARKER = False
DL_MODE = False 
...
# args1 = ((np.array([65, 45, 30]), np.array([100, 255, 255])),
#         (np.array([100, 35, 30]), np.array([113, 255, 255])),
#         (np.array([0, 0, 0]), np.array([0, 0, 0]))) #green + blue

args1 = ((np.array([0, 80, 130]), np.array([20, 255, 255])),
        (np.array([150, 80, 130]), np.array([179, 255, 255])),
...
while not rospy.is_shutdown():
    ...
    if currState == States.INIT:
        ...
        if abs(_the) > 40:
            ...
            robot.walkVelocities(x=30, y=0, th=np.clip(_the, -10, 10), z_move_amplitude=0.045, balance=True, z_offset=0)# y+:left,-:right the+:left, -:right
        ...
        # elif(not sea_the_line()):
        #     print("maybe loss from line")
        #     # robot.walkStop()
        #     # rospy.sleep(0.1)
        #     if(not sea_the_line()):
        #         # robot.walkVelocities(x = -0.1, y=0, th=0, z_move_amplitude=0.045, balance=True, z_offset=0)
        #         # rospy.sleep(0.5)
        #         # robot.walkVelocities(x = -10, y=0, th=0, z_move_amplitude=0.045, balance=True, z_offset=0)
        #         # rospy.sleep(1)
        #         robot.walkStop()
        #         print("loss from line")
        #         # while not robot.buttonCheck("start"):
        #         #     robot.walkStop()
        #         pass_time = time.time()
        #         while (time.time() - pass_time) < 1.5 :
        #             robot.setJointPos(["head_tilt"], [np.radians(-80)])
        #             caculate_theta_and_moving_head()
        #             # if vision.status[0]:
        #             #     print((vision.results[0]) , np.degrees(robot.joint_pos["head_pan"]))
        #         # robot.setJointPos(["head_tilt"], [np.radians(-60)])
        #         if vision.status[0]  and vision.results[0][1] > 500:
        #             _angle = vision.results[0][2] + np.degrees(robot.joint_pos["head_pan"])
        #         else:
        #             _angle = np.degrees(robot.joint_pos["head_pan"])
        #         print("set angle to "+str(_angle))
        #         # while not robot.buttonCheck("start"):
        #         #     robot.walkStop()
        #         # robot.setJointPos(["head_tilt"], [np.radians(-60)])
        #         robot.setJointPos(["head_tilt","head_pan"], [np.radians(-60),np.radians(0)])
        ...
        #         #     robot.walkStop()
        #         pass_time = time.time()
        #         while((time.time() - pass_time) < 2 and (not sea_the_line())):
        #             robot.walkVelocities(x = 15, y=0, th=np.clip(z_gyro, -10, 10), z_move_amplitude=0.045, balance=True, z_offset=0)
        #         # while not robot.buttonCheck("start"):
        #         #     robot.walkStop()
        #         if(not sea_the_line()):
        #             currState = States.FOLLOW_IMU
                ...
                # pass_time = time.time()
                # robot.walkStart()
                # while((time.time() - pass_time) < 3 and (not sea_the_line())):
                #     robot.walkVelocities(x = -0.1, y=0, th=np.clip(z_gyro, -10, 10), z_move_amplitude=0.045, balance=True, z_offset=0)
                # pass_time = time.time()
                # while((time.time() - pass_time) < 1 and (not sea_the_line())):
                #     robot.walkVelocities(x = 15, y=0, th=np.clip(z_gyro, -10, 10), z_move_amplitude=0.045, balance=True, z_offset=0)
                # # while not robot.buttonCheck("start"):
                # #     robot.walkStop()
                # if(not sea_the_line()):
        ...
        robot.walkStart()
        if(vision.status[1]):
            print("maybe is arrow(FOLLOW_ARROW)")
            robot.walkStop()
            pass_time = time.time()
            rospy.sleep(1)
        ...
        if abs(z_gyro) > 20:
            ...
            robot.walkVelocities(x=10, y=4, th=np.clip(z_gyro*1.1, -10, 10), z_move_amplitude=0.045, balance=True, z_offset=0)# y+:left,-:right the+:left, -:right
    # robot.walkVelocities(x=0, y=0, th=np.clip(z_gyro, -15, 15), z_move_amplitude=0.045, balance=True, z_offset=0)# y+:left,-:right the+:left, -:right
        if(vision.status[1]):
            print("maybe is arrow(FOLLOW_IMU)")
            robot.walkStop()
            pass_time = time.time()
