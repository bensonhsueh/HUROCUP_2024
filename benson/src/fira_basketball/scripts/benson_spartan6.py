...
while not rospy.is_shutdown():
    ...
    if currState == States.INIT:
        ...
        if robot.buttonCheck("start"):
            detect_line = []
            start_time = time.time()
            currState = States.STRAIGHT

# fdjoirjhoidthokdtfjmhgodfji
            
    elif currState == States.STRAIGHT:
        print("[STRAIGHT]")
        robot.walkStart()
        robot.walkVelocities(x=1, y=0, th=np.clip(z_gyro, -10, 10), z_move_amplitude=0.045, balance=True, z_offset=0)
