...
bool QNodeOP3::init()
{
  ...
  // imu_sub_ = ros_node.subscribe("/robotis_op3/imu", 10, &QNodeOP3::imuCallback, this);
  imu_sub_ = ros_node.subscribe("/robotis/open_cr/imu", 10, &QNodeOP3::imuCallback, this);
...
// IMU
void QNodeOP3::imuCallback(const sensor_msgs::Imu::ConstPtr &msg)
{
  ...
  // ROS_INFO("Roll : %3.2f, Pitch : %2.2f, Yaw : %2.2f", rpy_orientation.coeff(0, 0), rpy_orientation.coeff(1, 0), rpy_orientation.coeff(2, 0));

  Q_EMIT updateIMU(rpy_orientation.coeff(0, 0), rpy_orientation.coeff(1, 0), rpy_orientation.coeff(2, 0));
}
