      ...
      is_walking_(false)
{
  ...
  // IMU
  QObject::connect(&qnode_op3_, SIGNAL(updateIMU(double,double,double)), this, SLOT(updateIMU(double,double,double)));
...
void MainWindow::updateIMU(double roll, double pitch, double yaw)
{
  // ROS_INFO("Roll : %3.2f, Pitch : %2.2f, Yaw : %2.2f", roll, pitch, yaw);

  ui_.roll_textbox->setText(QString::number(roll));
  ui_.pitch_textbox->setText(QString::number(pitch));
  ui_.yaw_textbox->setText(QString::number(yaw));
}
