...
class Robot:
    ...
    def __init__(self):
        ...
        self.head_joint_state_pub = rospy.Publisher("/robotis/head_control/set_joint_states", 
            JointState, queue_size=1)
        self.module_control_pub   = rospy.Publisher('/robotis/enable_ctrl_module',                  
            String, queue_size=0)
        ...
        rospy.Subscriber("/robotis/cm_740/button", String, self.updateBtnStatus, 
                queue_size=1)
    ...
    def onlineWalkSetup(self, x=0.0, y=0.0, z=0.0, foot_dist=0.070, foot_height=0.05, 
        ...
        # ha, who know what they are thinking.
    ...
    def onlineWalkIniPose(self):
        ...
        yf = yaml.load(ini_file, yaml.FullLoader)
    ...
    def setJointPos_V(self, joints, pos,velocity):
        msg = JointState()
        msg.name = joints
        msg.position = pos
    ...
    def setHeadJointPos(self, joints, pos):
        msg = JointState()
        msg.name = joints
        msg.position = np.radians(pos)
        

        self.head_joint_state_pub.publish(msg)
    
    def setCtrlModule(self, msg):
    ...
    def walkVelocities(self, x=0.0, y=0.0, th=0, z_move_amplitude=None, balance=None,
    ...
    def setGrippersPos(self,gripper_name,value):
        self.setJointsControlModule(["l_gripper", "r_gripper"], ["none", "none"])

        if value>=0 and value <= 100:

            open_value = 100-value

            if gripper_name == "left":
                max_value = abs(0.5+1.2)
                gripper_pos = 0.5-max_value*open_value/100
                self.setJointPos(["l_gripper"], [gripper_pos])
                rospy.sleep(0.5)

            elif gripper_name == "right":
                max_value = abs(2.4-0.5)
                gripper_pos = 0.5+max_value*open_value/100
                self.setJointPos(["r_gripper"], [gripper_pos])
                rospy.sleep(0.5)
        else:
...
def getWalkingParams(file_name='param.yaml'):
    ...
    '''
    CONFIG_PATH = path.join(rospack.get_path("op2_walking_module"), "config")
    print(path.join(CONFIG_PATH, file_name))
